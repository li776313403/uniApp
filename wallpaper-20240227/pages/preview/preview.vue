	...
	<view class="previewLayout">
		<swiper circular="true" :current="wallIndexRef" @change="wallChange">
			<swiper-item v-for="(item, index) in wallListComputed" :key="item._id">
				<image v-if="wallReadedRef.includes(index)" :src="item.picurl" mode="aspectFill" @click="imageClick">
				</image>
			</swiper-item>
		</swiper>

		<view v-show="shwoInfoRef" class="mask">
			<view @click="backClick" class="goBack"
				:style="{ top: backIconTopComputed + 'px', lfet: dy_TitleLeftIconDistanceComputed + 'px' }">
					...
					<view class="content">
						...
						<view class="row">
							...
							<view class="value roteBox">
								<uni-rate class="rate" :value="showScoreComputed" readonly allow-half
									:touchable="false" />
		...
		<uni-popup ref="ratePanlRef" type="center" :is-mask-click="false" @change="scorePanleChange">
			<view class="ratePanl">
				...
				<view class="footer">
					<button @click="rateSubmitClick" type="default" size="mini" plain
						:disabled="scoreRef === 0 || previeWallComputed.userScore">
